                    ...
                    for temp in temps:
                        for dw in dws:
                            if "resnet" in student:
                                print(f"python train_chained_student.py --dataset {dataset} --model {student} --teacher {teacher_model} --checkpoint {teacher_path} --temp {temp} --dw {dw} --lr 0.1 --lr-decay-factor 0.1 --wd 5e-4 --train-batch-size 128 --schedule-steps 100 150 --epochs 200 --exp_name runid={idx} --chain_length {CHAIN_LENGTH}")
                            else:
                                print(f"python train_chained_student.py --dataset {dataset} --model {student} --teacher {teacher_model} --checkpoint {teacher_path} --temp {temp} --dw {dw} --lr 0.001 --wd 1e-4 --train-batch-size 128 --epochs 200 --exp_name runid={idx} --chain_length {CHAIN_LENGTH}")
                            idx += 1                        
